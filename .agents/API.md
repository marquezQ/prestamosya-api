...
PATCH  /api/users/me               → UserResponseDto (solo `name` es editable)
PATCH  /api/users/me/password      → { message } (requiere la contraseña actual)
```

> **Separación de responsabilidades:** `auth` = autenticación ("quién soy"); `users` = cuenta del usuario ("mantener mi perfil y contraseña"). Las rutas de administración del super admin (`GET/POST /users`, `PATCH /users/:id`, `POST /users/:id/reset-password`) se agregarán aquí en el futuro.
...
> - `riskIndicators`: Tasa de morosidad actual (`delinquencyRate`), capital principal en riesgo (`capitalAtRisk`), nuevos préstamos y clientes del mes, y préstamos saldados (`completedLoansCount`).
> - `monthlyBalance`: Ganancia neta del mes consultado (`netProfit = interestCollected - discountsGiven`), capital principal desplegado en calle (`capitalDeployed`, exactamente idéntico al Home Dashboard) y retorno sobre capital (`returnOnCapital = netProfit / capitalDeployed * 100`). Todo desglosado en BOB y USD sin mezcla de monedas.
>
